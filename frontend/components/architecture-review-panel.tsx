...
import { GitBranch, Layers3, Loader2, ShieldAlert } from 'lucide-react';
...
export function ArchitectureReviewPanel({ scan }: ArchitectureReviewPanelProps) {
  ...
  async function runReview(source: 'path' | 'import') {
    ...
    try {
      ...
      const result =
        source === 'path'
          ? await reviewArchitecture(activePath, files, focus.trim() || undefined)
          : await reviewImportedArchitecture(importId.trim(), files, focus.trim() || undefined);
      setReview(result);
    ...
    <section className="space-y-4">
      <div className="grid gap-3 lg:grid-cols-2">
        <form className="space-y-3" onSubmit={submitPath}>
          <label className="sr-only" htmlFor="architecture-review-repository-path">
            Repository path
          </label>
          <div className="flex gap-2">
            <input
              id="architecture-review-repository-path"
        ...
        <form className="flex gap-2" onSubmit={submitImport}>
          <label className="sr-only" htmlFor="architecture-review-import-id">
            Import ID
          </label>
          <input
            id="architecture-review-import-id"
            value={importId}
      ...
      <div className="grid gap-3 lg:grid-cols-[280px_1fr]">
        ...
        <textarea
          ...
          rows={7}
          placeholder="optional architecture focus"
      ...
      {review ? (
        <div className="grid gap-4 xl:grid-cols-[280px_1fr]">
          <aside className="space-y-3 rounded-lg border border-border bg-card p-4">
            ...
            <div className="grid grid-cols-2 gap-2 text-sm">
              ...
              <Stat label="Violations" value={review.stats.violation_count} />
            </div>
            ...
            <div className="space-y-2 p-4">
              ...
              {review.findings.length === 0 ? (
                <p className="text-sm text-muted-foreground">
                  No architecture findings for the supplied changes.
                </p>
              ) : null}
            </div>
            <div className="grid gap-2 border-t border-border p-4 md:grid-cols-2">
              {review.impacted_files.map((file) => (
                <div key={file.path} className="rounded-md bg-muted p-3">
                  <div className="flex items-start justify-between gap-3">
                    <p className="truncate text-sm font-medium">{file.path}</p>
                    <p className="text-xs uppercase text-accent">{file.layer}</p>
                  </div>
        ...
        <div className="rounded-lg border border-border bg-card p-6 text-center text-sm text-muted-foreground">
          Review proposed changes for architecture impact, boundary violations, and key file risk.
        </div>
      )}
    </section>
  );
...
function FindingCard({ finding }: { finding: ArchitectureReviewFinding }) {
  return (
